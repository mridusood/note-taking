...
def redd():
    ...
    e3 = Text(fr, width=45, yscrollcommand=scrollb.set,bg='light grey',fg='black')
    e3.pack(side=LEFT, fill=BOTH)

    e3.insert(INSERT,z)
    c = Button(top, text='BACK', width=15, height=2, bg='red', fg='white', command=top.destroy)
...
def butt():
    b = Button(frame5, text='Update', bg='black', fg='white', command=Up).grid(row=4,column=2)

    c = Button(frame5, text='Delete', bg='black', fg='white', command=dele).grid(row=5,column=2)

    d = Button(frame5, text='Read', bg='black', fg='white', command=redd).grid(row=6,column=2)

    e=Button(frame5, text='Sort', bg='black', fg='white', command=sot).grid(row=3,column=2)
