...
#conn.execute("CREATE TABLE Notess(ID INTEGER PRIMARY KEY AUTOINCREMENT,NAME TEXT NOT NULL,CONTENT TEXT NOT NULL,PRIORITY NOT NULL,PASSWD TEXT NOT NULL);")
conn.commit()
master=Tk()
...
def add():
    ...
    def save():
        ...
        conn.execute("INSERT INTO Notess(NAME,CONTENT,PRIORITY,PASSWD)VALUES(?,?,?,?)",(x,input,m,t))
        conn.commit()
...
def liste():
    myLis.delete(0,END)
    cursor=conn.execute("SELECT ID,NAME from Notess")
    for row in cursor:
...
def search():
    myLis.delete(0, END)
    z=str(v.get())
    cursor = conn.execute("SELECT ID,NAME from Notess where NAME=?",(z,))
    for row in cursor:
        myLis.insert(END, str(row))
    butt()
...
def dele():
    ...
    z = conn.execute("SELECT PASSWD from Notess where ID=?", (del_id,))
    for row in z:
        n=row
    ...
    if(n[0][0:]!='None'):
        ...
        def ok():
            if(n[0][0:]==u.get()):
                ...
                conn.execute("DELETE FROM Notess where ID=?", (del_id,))
                conn.commit()
                liste()
            else:
...
def Up():
    def c():
        ...
        l = conn.execute("SELECT NAME,PRIORITY,CONTENT,PASSWD FROM Notess where ID=?", (del_id,))
        ...
        def save():
            ...
            conn.execute("INSERT INTO Notess(NAME,CONTENT,PRIORITY,PASSWD)VALUES(?,?,?,?)", (x, input, m, re))
            conn.commit()
...
def redd():
    def red():
        ...
        l=conn.execute("SELECT CONTENT FROM Notess where ID=?", (del_id,))
...
def sot():
    ...
    cursor=conn.execute("SELECT ID,NAME FROM Notess ORDER BY PRIORITY ASC")
